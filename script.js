...
let liveDraftWriteGen = 0;
let liveDraftThrottleTimer = null;
let liveDraftThrottlePending = false;
...
async function syncLiveDraft() {
  ...
  try {
    ...
    if (gen === liveDraftWriteGen) {
      liveDraftActive = true;
    }
  } catch (err) {
    console.error(
      "Live draft sync failed:",
      err
    );
  }
}

function scheduleLiveDraftSync() {
  if (
    liveDraftThrottleTimer !==
    null
  ) {
    liveDraftThrottlePending =
      true;
    return;
  }
  ...
  liveDraftThrottleTimer =
    setTimeout(() => {
      ...
      if (liveDraftThrottlePending) {
        liveDraftThrottlePending =
          false;
        scheduleLiveDraftSync();
      }
    }, 300);
}
